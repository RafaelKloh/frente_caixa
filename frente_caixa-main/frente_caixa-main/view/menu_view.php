<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="/frente_caixa-main/global_style.css">
    <link rel="stylesheet" href="/frente_caixa-main/menu.css">
</head>
<body>
    <ul>
        <li><a href="render_register_item">Cadastrar Item</a></li>
        <li><a href="render_register_client">Cadastrar Cliente</a></li>
        <li><a href="render_register_payment_method">Cadastrar Forma de Pagamento</a></li>
        <li><a href="render_register_promotion">Cadastrar Promoção</a></li>
        <li><a href="render_register_user">Cadastrar Usuário</a></li>
        <li><a href="render_list_promotion">Listar Promoções</a></li>
    </ul>
</body>
</html>